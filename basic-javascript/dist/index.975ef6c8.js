...
const { connect, disconnect } = require("2b5f8dba2d556015");
const axios = require("a4df52acf03d00cd").default;
video = document.getElementById("ai-video");
status_node = document.getElementById("status");
btn = document.getElementById("start");
DIARUPT_API = "https://engine.diarupt.ai";
get_media_stream = async ()=>{
    if (navigator.mediaDevices === undefined || navigator.mediaDevices.getUserMedia === undefined) throw new Error("navigator.mediaDevices is undefined");
    else {
...
parcelHelpers.export(exports, "disconnect", ()=>U);
var v = "https://engine.diarupt.ai";
var y = async ({ timeslice: t, stream: r }, n)=>{
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        r.getVideoTracks().forEach((i)=>{
            r.removeTrack(i);
        });
        let o = new MediaRecorder(r);
        return o.addEventListener("dataavailable", (i)=>{
            i.data.size > 0 && o?.state !== "inactive" && n(i.data);
        }), o.start(t ?? 250), o;
    } else return Promise.reject(new Error("mediaDevices API or getUserMedia method is not supported in this browser."));
}, k = async (t)=>await (await fetch(`${v}/secret-key/${t}`, {
        headers: {
            "Content-Type": "application/json"
        },
        method: "GET"
    })).json(), T = (t, r)=>{
    let n = "", e = null, o = async (i)=>{
        let d = new URLSearchParams({
            ...
            endpointing: "0",
            utterance: "false"
        }).toString(), { secret_key: a } = await k(t), c = new WebSocket("wss://api.deepgram.com/v1/listen?" + d, [
            "token",
            a
        ]), f = window;
        f.diarupt_stt_socket = c, c.onopen = ()=>{
            y({
                timeslice: 50,
                audio: !0,
                video: !1,
            ...
            u !== "" && u !== void 0 && (m.is_final && (n += ` ${u}`), i(u, !1), e && clearTimeout(e), e = setTimeout(()=>{
                i(n, !0), n = "";
            }, 550));
        }, c.onclose = (l)=>{
            console.log("socket closed", l), c.close(), setTimeout(()=>{
                o(i);
            }, 200);
        }, c.onerror = (l)=>{
            console.error({
...
var s = null, g = null, h = null, R = v;
function _(t, r) {
    let n = {
        sdpSemantics: "unified-plan"
    };
    return t && (n.iceServers = [
        {
            urls: [
                "stun:stun.l.google.com:19302"
            ]
        }
    ]), s = new RTCPeerConnection(n), s.addEventListener("icegatheringstatechange", function() {
        console.log("icegatheringstatechange", s?.iceGatheringState);
    }, !1), s.addEventListener("iceconnectionstatechange", function() {
        console.log("iceconnectionstatechange", s?.iceConnectionState);
    }, !1), s.addEventListener("signalingstatechange", function() {
        console.log("signalingstatechange", s?.signalingState);
    }, !1), s.addEventListener("track", function(e) {
        console.log("track", e), e.track.kind, r.srcObject = e.streams[0];
    }), s;
}
async function C(t, r = {
    ...
    let n = await s?.createOffer({
        offerToReceiveVideo: !0,
        offerToReceiveAudio: !0
    });
    await s?.setLocalDescription(n);
    let e = async ()=>{
        n = s?.localDescription, r.audio !== "default" && (n.sdp = S("audio", r.audio, n.sdp ?? "")), r.video !== "default" && (n.sdp = S("video", r.video, n.sdp ?? ""));
        let o = await fetch(R + "/offer", {
            body: JSON.stringify({
                sdp: n.sdp,
                type: n.type,
                session_id: t
        ...
        let i = await o.json();
        return s?.setRemoteDescription(i);
    };
    return new Promise((o, i)=>{
        if (s?.iceGatheringState === "complete") e().then(o).catch(i);
        else {
            let d = ()=>{
                s?.iceGatheringState === "complete" && (s?.removeEventListener("icegatheringstatechange", d), e().then(o).catch(i));
            };
            s?.addEventListener("icegatheringstatechange", d);
        }
    });
}
async function E(t, r, n, e) {
    try {
        ...
        let o = window || {};
        o.peer = s;
        let i = null, d = ()=>i === null ? (i = new Date().getTime(), 0) : new Date().getTime() - i;
        return g = s.createDataChannel("chat", {
            ordered: !0
        }), g.onclose = function() {
            e("close", "data channel closed"), h && clearInterval(h), console.log(`- close peer connection
`);
        }, g.onopen = function() {
            o.peer_dc = g, e("open", "data channel open"), h = setInterval(function() {
                var a = "ping " + d();
                g?.send(a);
            }, 1e3);
            ...
            e("error", a), o.peer_dc = null;
        }, n.stream.getTracks().forEach((a)=>{
            s?.addTrack(a, n.stream);
        }), await C(t, n.codecs);
    } catch (o) {
        return console.log(o), e("error", o), null;
    }
}
function S(t, r, n) {
    for(var e = [], o = new RegExp("a=fmtp:(\\d+) apt=(\\d+)\\r$"), i = new RegExp("a=rtpmap:([0-9]+) " + L(r)), d = new RegExp("(m=" + t + " .*?)( ([0-9]+))*\\s*$"), a = n.split(`
`), c = !1, f = 0; f < a.length; f++)if (a[f].startsWith("m=" + t + " ") ? c = !0 : a[f].startsWith("m=") && (c = !1), c) {
        var l = a[f].match(i);
        l && e.push(parseInt(l[1])), l = a[f].match(o), l && e.includes(parseInt(l[2])) && e.push(parseInt(l[1]));
    }
    var m = "a=(fmtp|rtcp-fb|rtpmap):([0-9]+)", u = "";
    c = !1;
    for(var p = 0; p < a.length; p++)if (a[p].startsWith("m=" + t + " ") ? c = !0 : a[p].startsWith("m=") && (c = !1), c) {
        var w = a[p].match(m);
        if (w && !e.includes(parseInt(w[2]))) continue;
        a[p].match(d) ? u += a[p].replace(d, "$1 " + e.join(" ")) + `
` : u += a[p] + `
`;
    } else u += a[p] + `
`;
...
var O = async (t, r, n)=>{
    try {
        await E(t, r.player, {
            ...
            n(e, o), e === "open" && (await T(t, r.stream))((d, a)=>{
                console.log(`${new Date} > Final: ${d}`, a);
                let c = window;
                c.peer_dc && c.peer_dc.send(JSON.stringify({
                    type: "talk",
                    data: {
                        response: d,
                        responseIndex: 0,
    ...
    let t = window;
    t.peer && (t.peer.close(), t.peer = null, t.peer_dc = null), t.diarupt_stt_socket && (t.diarupt_stt_socket.close(), t.diarupt_stt_socket = null);
};
...
},{"96f156f3914c0f84":"bLbo2","3b69743a32808809":"jpEth"}],"bLbo2":[function(require,module,exports) {
"use strict";
...
},{}],"jpEth":[function(require,module,exports) {
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["igKGj","8lqZg"], "8lqZg", "parcelRequirec570")
